...
const TestimonialCard = ({ content, author, position, company, contact }: TestimonialCardProps) => {
  const [expanded, setExpanded] = useState(false);

  // If content is long, truncate it for initial display
  const isLongContent = content.length > 200;
  const truncatedContent = isLongContent && !expanded ? content.slice(0, 200) + '...' : content;
